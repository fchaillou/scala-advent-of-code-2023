...
object Day1 extends IOApp.Simple {
  ...
  private val longestNumberAsLetter = lettersAsNumber.keys.map(_.length).max
      ...
      line.toCharArray.toVector.foldRight(List.empty[String | Int]) {
        (prevChar, list) =>
          list match
            ...
            case ::(head: String, next) if !prevChar.isDigit => s"$prevChar$head" :: next
            case ::(head: String, next) if prevChar.isDigit => prevChar.asDigit :: head :: next
            case ::(head: Int, next) if prevChar.isDigit => prevChar.asDigit :: head :: next
